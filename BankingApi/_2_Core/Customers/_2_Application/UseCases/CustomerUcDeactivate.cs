...
public sealed class CustomerUcDeactivate(
   ...
   public async Task<Result> ExecuteAsync(
      ...
      var result = customer.Deactivate(employeeId, now);
      if (result.IsFailure)
         return Result.Failure(result.Error)
            .LogIfFailure(logger, "CustomerUcDeactivate.DomainRejected", new { customerId, employeeId, now });

      // 5) Persist
      var savedRows = await unitOfWork.SaveAllChangesAsync("Customer deactivated by employee", ct);
      logger.LogInformation("Customer deactivated customerId={customerId} savedRows={rows}", customerId, savedRows);
